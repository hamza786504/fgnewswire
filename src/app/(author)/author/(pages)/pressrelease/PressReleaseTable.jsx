...
export default function PressReleaseTable({ initialPressReleases }) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://api.glassworld06.com/api/press-releases/${selectedPressRelease.id}`, {
        method: 'DELETE',
        headers: {
                  ...
                  <tr key={pr.id}>
                    ...
                    <td className="px-6 py-4 text-sm">
                      <div className="flex gap-2">
                        <Link 
                          href={`/author/pressrelease/edit/${pr.slug || pr.id}`}
