...
export default function EditGuestPost() {
  ...
  const [isApproved, setIsApproved] = useState(false); // State for approval toggle
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Set approval status based on current status
        setIsApproved(pressData.status === "approved");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Send all editable fields including status
      formDataToSend.append("title", formData.title);
      formDataToSend.append("body", formData.body);
      
      // Set status based on approval toggle
      ...
      <main className="flex-1 bg-[#ebecf0] min-h-full p-4 md:pb-6 md:px-4">
        <div className="max-w-5xl mx-auto bg-white rounded-3xl p-4 md:p-6">
          <div className="flex justify-center items-center py-12">
            ...
            <span className="ml-2 text-gray-600">Loading Guest Post details...</span>
          </div>
        </div>
      </main>
    );
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-3xl p-4 md:p-6">
        ...
        <form onSubmit={handleSubmit}>
          <div className="flex justify-between items-center mb-5">
            <h2 className="text-lg font-semibold text-gray-900">Edit Guest Post</h2>
          </div>
          ...
          <div className="mb-5 p-4 bg-gray-50 rounded-lg border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <label className="block text-sm font-medium text-gray-900">Approval Status</label>
                <p className="text-xs text-gray-500 mt-1">
                  {isApproved ? "This press release is APPROVED and will be published" : "This press release is in DRAFT mode and not visible to public"}
                </p>
              </div>
              <div className="flex items-center gap-3">
                <span className={`text-sm ${!isApproved ? "text-gray-900 font-medium" : "text-gray-500"}`}>
                  Draft
                </span>
                <button
                  type="button"
                ...
                <span className={`text-sm ${isApproved ? "text-green-600 font-medium" : "text-gray-500"}`}>
                  Approved
                </span>
              </div>
            </div>
            {isApproved && (
              <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded-md">
                <p className="text-xs text-green-800">
                  ⚠️ When approved, this press release will be visible to the public immediately upon update.
                </p>
              </div>
            )}
            {!isApproved && (
              <div className="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded-md">
                <p className="text-xs text-yellow-800">
                  ℹ️ Draft mode means this press release is saved but not visible to the public.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="mt-6 flex justify-end gap-3">
            <button
              type="button"
              onClick={() => router.push("/author/guest-post")}
