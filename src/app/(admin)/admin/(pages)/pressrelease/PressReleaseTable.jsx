...
export default function PressReleaseTable({ initialPressReleases }) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/press-releases/${selectedPressRelease.slug}`, {
        method: 'DELETE',
        headers: {
                  ...
                  <tr key={pr.id}>
                    ...
                    <td className="px-6 py-4 text-sm">
                      <div className="flex gap-2">
                        <Link
                          href={`/admin/pressrelease/edit/${pr.slug}`}
                        ...
                        <button
                          ...
                          className="bg-red-600 text-white px-3 py-1 rounded-md text-xs hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
