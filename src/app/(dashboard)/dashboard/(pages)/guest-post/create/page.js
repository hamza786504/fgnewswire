            ...
            {loadingOrders ? (
              ...
              <>
                <select
                  ...
                  className={`mt-1 block w-full rounded-md border px-3 py-2 ${errors.order_item_id ? "border-red-500" : "border-gray-300"
                    }`}
                >
                ...
                {availableOrders
  ?.filter((order) => order.item?.item_type === "guest_posting")
  .map((order) => (
      ...
      {order.item.name}
          ...
          <div className="mt-6 flex justify-end gap-3">
            ...
            <button
              ...
              className={`px-5 py-2 rounded-lg text-white bg-gradient-to-r from-blue-500 to-purple-700 ${isSubmitting || availableOrders.length === 0
                  ? "opacity-50 cursor-not-allowed"
                  : "hover:from-blue-600 hover:to-purple-800 transition-all"
                }`}
            >
              {isSubmitting ? "Creating..." : "Submit Guest Post"}
            </button>
          </div>
        </form>
      </div>
